...
import { useGetBookedEventQuery } from "../../data/events";
            ...
            <VStack space={2}>
              <VStack space={2}>
                <Image
                  ...
                  source={{ uri: imageUrlResolver(data?.eventImages[0] ?? "") }}
                  alt="Image"
                />
              ...
              <VStack space={1}>
                <HStack space={2} alignItems={"center"}>
                  ...
                  <Text fontWeight={"400"} fontSize={12} color={colors.primary}>
                    {`${
                      data?.eventStart
                        ? dayjs(new Date(data?.eventStart ?? ""))?.format(
                            "DD MMMM h:mmA"
                          )
                        : ""
                    } - ${
                      data?.eventEnd
                        ? dayjs(new Date(data?.eventEnd ?? ""))?.format(
                            "DD MMMM h:mmA"
                          )
                        : ""
                    }`}
